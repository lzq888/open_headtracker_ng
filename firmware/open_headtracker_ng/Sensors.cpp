...
// Local variables
//
byte sensorBuffer[14];       // Buffer for bytes read from sensors
...
void UpdateSensors()
{
    ...
    ReadFromI2C(MPU_6050_ADDR, MPU_6050_X_ADDR, 14);
    accRaw[0] = ((int)sensorBuffer[1] | ((int)sensorBuffer[0] << 8)) * -1;
    accRaw[1] = ((int)sensorBuffer[3] | ((int)sensorBuffer[2] << 8)) * -1;       
    accRaw[2] = (int)sensorBuffer[5] | ((int)sensorBuffer[4] << 8);       
          
    gyroRaw[0] = (int)sensorBuffer[9] | ((int)sensorBuffer[8] << 8);
    gyroRaw[1] = ( (int)sensorBuffer[11] | ((int)sensorBuffer[10] << 8) ) * -1;
    gyroRaw[2] = ( (int)sensorBuffer[13] | ((int)sensorBuffer[12] << 8) ) * -1;
...
void InitSensors()
{
    ReadFromI2C(MPU_6050_ADDR, 0x00, 1);
    MPU_6050_ID = sensorBuffer[0];
    ...
    // MPU6050
    // wake up
    WriteToI2C(MPU_6050_ADDR, 0x6B, 0);
...
{ 
   ...
   Serial.print("MPU-6050 ID: ");
   Serial.println((int)MPU_6050_ID); 
